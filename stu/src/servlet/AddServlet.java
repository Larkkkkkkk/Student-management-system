		...
		//2.添加到数据库	
			try {
					//birthday需要从string转date
						...
						Student student=new Student(sname,gender,phone,hobby,info,date);  //然后给一个student对象  里面用有参构造方法去将客户端提交的数据存下来
	
						StudentService service=new StudentServiceImpl();	 //然后通过service去找dao解决
						
				try {
							service.insert(student);  //service找它的insert方法 --service的实现类又要去找dao
